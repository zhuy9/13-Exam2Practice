...
Authors: David Mutchler, Valerie Galluzzi, Mark Hays, Amanda Stouder,
         their colleagues and YUCHEN ZHU @@DZ
...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        i = (self.volume - len(self.contents))
        add_string = additional_contents[0: i]
        remain_string = additional_contents[i: len(additional_contents)]
        self.contents += add_string
    ...
    def shrink(self, new_volume):
        ...
        remain = self.contents[new_volume: len(self.contents)]
    ...
    def double_then_shrink(self, new_volume):
        ...
        # --------------------------------------------------------------
        '''
        if self.o_vulume - len(self.contents)*2 <= 0:
            a = abs(len(self.contents)*2 - self.o_vulume)
        else:
            a = 0
        self.double()
        # print(self.contents)
        # print(a)
        if  new_volume - len(self.contents) <= 0:
            b = abs(len(self.contents) - new_volume)
        else:
            b = 0
        self.contents = self.contents[0:new_volume]
        # print(self.contents)
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        self.saved += [self.contents]
        self.volume = self.o_vulume

        if len(self.o_contents) < self.volume:
    ...
    def steal(self, other_box):
        ...
        other_box.contents = other_box.contents [self.volume - len(self.o_contents): len(other_box.o_contents)]
